...
bool SearchRight(char *word, int size, char **tablero, char *res) {
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
            if (j > size - strlen(word)) break; // Para no tener que buscar más de la cuenta
            int k = 0, l = j;
...
bool SearchLeft(char *word, int size, char **tablero, char *res) {
    for (int i = 0; i < size; i++) {
        for (int j = size - 1; j >= 0; j--) {
            if (j < strlen(word) - 1) break; // Para no tener que buscar más de la cuenta
            int k = 0, l = j;
            bool centinela = false;
            while (word[k] != '\0') {
                if (tablero[i][l] != word[k]) {
...
bool SearchDown(char *word, int size, char **tablero, char *res) {
    for (int j = 0; j < size; j++) {
        for (int i = 0; i < size; i++) {
            if (i > size - strlen(word)) break; // Para no tener que buscar más de la cuenta
...
bool SearchUp(char *word, int size, char **tablero, char *res) {
    for (int j = 0; j < size; j++) {
        for (int i = size - 1; i >= 0; i--) {
            if (i < strlen(word) - 1) break; // Para no realizar busquedas imnecesarias
